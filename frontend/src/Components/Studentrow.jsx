...
const StudentRow = ({ student, index, isDarkMode }) => {
  ...
  const getRatingBadgeColor = (rating) => {
    ...
    if (rating < 1400) return isDarkMode ? 'bg-green-900 bg-opacity-30' : 'bg-green-100'
    if (rating < 1600) return isDarkMode ? 'bg-teal-900 bg-opacity-30' : 'bg-teal-100'
    if (rating < 1900) return isDarkMode ? 'bg-blue-900 bg-opacity-30' : 'bg-blue-100'
    if (rating < 2100) return isDarkMode ? 'bg-purple-900 bg-opacity-30' : 'bg-purple-100'
    if (rating < 2400) return isDarkMode ? 'bg-yellow-900 bg-opacity-30' : 'bg-yellow-100'
    return isDarkMode ? 'bg-red-900 bg-opacity-30' : 'bg-red-100'
  }

  return (
    <tr className={`border-b transition ${isDarkMode ? 'hover:bg-gray-700 border-gray-700' : 'hover:bg-gray-50 border-gray-200'}`}>
      ...
      <td className="px-6 py-4">
        <div className="flex items-center">
          <a
            ...
            className={`font-medium ${getRatingColor(student.CurrRating)} hover:underline`}
          >
            {student.cfhandle}
          </a>
          {student.CurrRating && (
            <span className={`ml-2 text-xs px-2 py-1 rounded ${getRatingBadgeColor(student.CurrRating)} ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}>
              {student.CurrRating}
            </span>
          )}
        </div>
