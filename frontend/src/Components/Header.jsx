...
import { FiSun, FiMoon, FiDownload, FiEdit2, FiUserPlus, FiLogIn, FiLogOut } from 'react-icons/fi';
...
const Header = ({ setIsSubmiting, setSubmitindStudentId }) => {
  ...
  return (
    <>
      <header className={`flex justify-between items-center p-4 sticky top-0 z-40 transition-colors duration-300 ${
        isDarkMode 
        ...
        <div className='flex gap-3 items-center'>
          ...
          {isAuthenticated && (
            ...
            >
              {isLoading ? (
                ...
                <>
                  {hasStudentData ? <FiEdit2 className="w-5 h-5" /> : <FiUserPlus className="w-5 h-5" />}
          ...
          <button
            ...
            onClick={handleAuth}
          >
            {isAuthenticated ? <FiLogOut className="w-5 h-5" /> : <FiLogIn className="w-5 h-5" />}
